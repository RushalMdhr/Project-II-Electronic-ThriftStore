  ...
  const getActionButton = (order) => {
    if (order.status === "confirmed") {
      return (
        <button
          onClick={() =>
            takeAction({ orderId: order._id, action: "processing" })
          }
      ...
      return (
        <>
          <button
            onClick={() =>
              takeAction({ orderId: order._id, action: "confirmed" })
            }
          ...
          <button
            onClick={() =>
              takeAction({ orderId: order._id, action: "cancelled" })
            }
            className="px-3 py-1 bg-red-500 hover:bg-red-500 mx-2 text-white rounded-md text-sm font-medium transition-colors"
          >
            Cancel
          </button>
        </>
          ...
          <table className="min-w-full divide-y divide-gray-700">
            <thead className="bg-gray-800">
              <tr>
                <th
                  scope="col"
                  className="px-6 py-3 text-left text-sm font-semibold text-emerald-400 uppercase tracking-wider"
                >
                  Order ID
                </th>
                <th
